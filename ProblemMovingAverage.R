...
movingAverage<-function(average_vector,sd_vector,data,windowsize,threshold,column){
  ...
  for (i in initial:length(y_new)){
    ...
    else if( diff>threshold){   #can replace threshold by x_1[index]
...
DataDf <- read.table("/Users/zhouenwei/Desktop/train.txt", header = T, sep = ",")
...
summerday<-DataDf[which(DataDf$Month>=5 & DataDf$Month<=8  & DataDf$Hour>=6 & DataDf$Hour<21),]#6:00am- 8:59pm
...
x_1<-rollapply(p_a,width=15,by=14,FUN=sd,align="left")
update<-data.frame(x)
...
d<-movingAverage(update,update_1,summer,15,0.7,'Global_active_power')
